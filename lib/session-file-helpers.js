...
var helpers = {
  ...
  mapPath: function (options) {
    return path.join(options.path, options.mapName + options.fileExtension);
  },

  getIdentity: function ( session, options ) {
    let identity;
    ...
    if ( options.identityField && Array.isArray(options.identityField) ) {
      for ( let keyItem of options.identityField ) {
        identity = identity ? safe( identity, keyItem ) : safe( session, keyItem )
      }
    } else if ( options.identityField instanceof String ) {
      identity = safe( session, options.identityField )
    }
  ...
  DEFAULTS: {
    ...
    encryptEncoding: 'hex',
    mapName: 'session-map',
    identityField: [ 'passport', 'user', 'username' ],
    fileExtension: '.json',
    crypto: {
  ...
  updateMap: function (sessionId, identity, options) 
  {
    var mapPath = helpers.mapPath(options);

    fs.readFile( mapPath, { encoding: options.encoding, flag: 'a+' }, function readCallback(err, data) 
    {
      if (!err) 
      {
        var map = helpers.decodeJSON( data );

        if ( identity&&(!map[identity]||map[identity]!==sessionId) ) {
          map[identity] = sessionId;
        } else if ( !identity ) {
          var record = Object.keys( map ).find( k => map[k] == sessionId );
          if ( record !== undefined ) {
            delete map[ record ];
          }
        }
  ...
  findSessionByIdentity: function ( identity, options, callback, idOnly )
  {
    var mapPath = helpers.mapPath(options);
      ...
      if (!err) {
        ...
        if ( sessionId !== undefined ) {
          ...
          if (idOnly) {
            ...
            helpers.get( sessionId, options, callback )
          }
  ...
  findIdentityBySessionId: function ( sessionId, options, callback )
  {
    var mapPath = helpers.mapPath(options);
    ...
    fs.readFile( mapPath, { encoding: options.encoding, flag: 'a+' }, function readCallback(err, data) {
      ...
      if (!err) {
        ...
        var map = helpers.decodeJSON( data );
        var identity = Object.keys( map ).find( id => map[id] === sessionId );
  ...
  set: function (sessionId, session, options, callback) {
    try {
      var identity = helpers.getIdentity( session, options )
      
      if (identity) {
        helpers.updateMap(sessionId, identity, options);
      }
  ...
  destroy: function (sessionId, options, callback) {
    ...
    helpers.updateMap( sessionId, null, options )
  },
  ...
  list: function (options, callback) {
    fs.readdir(options.path, function (err, files) {
      ...
      console.log( 'readdir result', err, files )
